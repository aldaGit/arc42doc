...
public class CoreDatabase implements AutoCloseable {
  ...
  private static final String RATIONALE_ID = "rationaleId";
  private static final String BAUSTEINSICHT_IMAGE =
      " (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision)-[c:wirdBegründetDurch]->(z)";
  private static final String MATCH_RATIONALE =
      "match(a:Arc42) match(z:Rationale) where Id(a)=$id and z.id=$rationaleId match";
  private static final String COUNT_S2 = " count(s)";
  private static final String MATCH_ID = "match(a:Arc42) where Id(a)=$id match";
  private static final String S_DESCRIPTION = "s.description";
  private static final String RETURN_COUNT_A = " return count(a)";
  private static final String COUNT_S = "count(s)";
  private static final String RETURN_COUNT_S = " return count(s)";
  private static final String COUNT_A = "count(a)";
  private final Driver driver;

  public CoreDatabase(String uri, String user, String password) {
  ...
  public int countAufgabenstellung(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          ...
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_A).asInt();
          }
          return 0;
        });
  }
  ...
  public int countStakeholder(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          ...
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_S).asInt();
          }
          return 0;
        });
  }
  ...
  public int countFachlicherKontext(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:kontext]->(s:Kontext:FachlichKontext) return count(s)",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_S).asInt();
  ...
  public String getFachlicherKontext(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:kontext]->(s:Kontext:FachlichKontext) return s.fkontext",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
  ...
  public String getTechnischerKontext(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:kontext]->(s:Kontext:TechnischhKontext) return s.tkontext",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
  ...
  public String getBausteinBeschreibung(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          ...
          if (result != null) {
            Record single = result.single();
            return single.get(S_DESCRIPTION).asString();
          }
          return "";
        });
  }
  ...
  public int countVerteilungDiagram(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:verteilungssicht]->(s:Image:Verteilung) return count(s)",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_S).asInt();
  ...
  public String getVerteilungBeschreibung(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:verteilungssicht]->(s:Image:Verteilung) return s.description",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
            return single.get(S_DESCRIPTION).asString();
  ...
  public int countArchitekturEntscheidung(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:hatEntscheidung]->(s:EntwurfsEntscheidung) return count(s)",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_S).asInt();
  ...
  public String getDecisionIdForRationaleId(Integer arcId, String rationaleId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_RATIONALE + BAUSTEINSICHT_IMAGE + " return v.id",
                  parameters("id", arcId, RATIONALE_ID, rationaleId));
          if (result != null) {
        ...
        transaction -> {
          Result result =
              transaction.run(
                  ...
                  parameters("id", arcId));
          if (result != null) {
            return new ArrayList<>(result.list(r -> r.get("z.id").asString()));
          }
          return null;
        });
  }
  ...
  public String getRationaleBeschreibung(Integer arcId, String rationaleId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_RATIONALE + BAUSTEINSICHT_IMAGE + " return z.beschreibung",
                  parameters("id", arcId, RATIONALE_ID, rationaleId));
          if (result != null) {
  ...
  public String getRationaleReferenz(Integer arcId, String rationaleId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_RATIONALE + BAUSTEINSICHT_IMAGE + " return z.referenz",
                  parameters("id", arcId, RATIONALE_ID, rationaleId));
          if (result != null) {
  ...
  public int countQualitaetSzenarien(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  MATCH_ID + " (a)-[r:hasQualityScenario]->(s:Qualitaetsscenario) return count(s)",
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
            return single.get(COUNT_S).asInt();
  ...
  public int countRisiken(Integer arcId) {
    ...
    return session.writeTransaction(
        transaction -> {
          Result result =
              transaction.run(
                  "match(a:Arc42) where Id(a)=$id match (a)-[r:hatRisiken]->(s:Risiken) return"
                      + COUNT_S2,
                  parameters("id", arcId));
          if (result != null) {
            Record single = result.single();
              ...
              Result result =
                  transaction.run(
                      MATCH_ID
                          ...
                          + " (a)-[r:analyseergebnisse]->(s:AnalyseErgebnis)"
                          + System.lineSeparator()
                          + RETURN_COUNT_S,
                      parameters("id", arcId));
              Integer count = null;
        ...
        return session.writeTransaction(
            transaction -> {
              ...
              if (aElementsResult != null) {
                return aElementsResult.list(r -> r.get("a.name").asString().trim()).stream()
